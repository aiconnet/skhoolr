...
class Mark extends CI_Controller
{
    ...
    function index()
    {
        ...
        $data['marks'] = $this->Mark_model->get_all_marks($params = array());

        $data['view'] = 'mark/index';
    ...
    */
    ...
    public function import_data()
    {
        ...
        if ($this->upload->do_upload('file')) {
            ...
            $this->db->insert_batch('marks', $data_excel);
             $this->session->set_flashdata('msg', 'Student Marks Registered Successfully!');
    ...
    function edit($id)
    {
        ...
        $data['mark'] = $this->Mark_model->get_mark($id);
        ...
        if (isset($data['mark']['id'])) {
            ...
            if ($this->form_validation->run()) {
                ...
                $this->Mark_model->update_mark($id, $params);
                redirect('mark/index');
            } else {
                $this->load->model('Student_model');
                $data['all_students'] = $this->Student_model->get_all_students();
    ...
    function remove($id)
    {
        $mark = $this->Mark_model->get_mark($id);

        // check if the mark exists before trying to delete it
        if (isset($mark['id'])) {
            $this->Mark_model->delete_mark($id);
            redirect('mark/index');
        } else
