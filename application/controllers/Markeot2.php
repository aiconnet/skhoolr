...
class Markeot2 extends CI_Controller
{
    ...
    function index()
    {
        ...
        // $config['total_rows'] = $this->Mark_model->get_all_marks_count();
        // $this->pagination->initialize($config);

        $data['marks'] = $this->Markeot2_model->get_all_marks($params = array());

        $data['view'] = 'markeot2/index';
    ...
    */
    ...
    public function import_data_eot2()
    {
        $config = array(
        ...
        if ($this->upload->do_upload('file')) {
            ...
            for ($i = 2; $i <= $sheets['numRows']; $i++) {
                ...
                $data_excel[$i - 1]['term'] = $sheets['cells'][$i][5];
                $data_excel[$i - 1]['subject'] = $sheets['cells'][$i][6];
                $data_excel[$i - 1]['subjectcode'] = $sheets['cells'][$i][7];
                $data_excel[$i - 1]['mark1'] = $sheets['cells'][$i][8];
                $data_excel[$i - 1]['mark2'] = $sheets['cells'][$i][9];
                $data_excel[$i - 1]['average_mark'] = $sheets['cells'][$i][10];
                $data_excel[$i - 1]['comment'] = $sheets['cells'][$i][11];
                $data_excel[$i - 1]['subjectteacher'] = $sheets['cells'][$i][12];
           ...
           // $this->db->insert_batch('marks', $data_excel);
             $this->db->insert_batch('full_marks', $data_excel);
             $this->session->set_flashdata('msg', 'Students Marks Registered Successfully!');
            // @unlink($data['full_path']);
            //redirect('excel_import');
            redirect('markeot2/index');
        }
    }
    ...
    function edit($id)
    {
        ...
        $data['mark'] = $this->Markeot2_model->get_mark($id);
        ...
        if (isset($data['mark']['id'])) {
            ...
            if ($this->form_validation->run()) {
                ...
                $this->Markeot2_model->update_mark($id, $params);
                redirect('markeot2/index');
            } else {
                $this->load->model('Student_model');
                $data['all_students'] = $this->Student_model->get_all_students();
    ...
    function remove($id)
    {
        $mark = $this->Markeot2_model->get_mark($id);

        // check if the mark exists before trying to delete it
        if (isset($mark['id'])) {
            $this->Markeot2_model->delete_mark($id);
            redirect('markeot2/index');
        } else
